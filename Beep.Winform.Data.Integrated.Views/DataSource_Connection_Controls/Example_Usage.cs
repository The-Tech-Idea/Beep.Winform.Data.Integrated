...
namespace TheTechIdea.Beep.Winform.Default.Views.Examples
{
    ...
    public class ConnectionDialogExamples
    {
        ...
        public void CreateNewConnectionExample()
        {
            ...
            // Skill § 1: dialog sizing flows from BeepLayoutMetrics; token is the single source of truth.
            // Skill § 9.1: this is user-requested "OK/Cancel" host code, NOT layout design-time content.
            // Note: the calling class isn't a Control, so we use the token literal and rely on
            // the form's AutoScaleMode (set in the BeepDialogManager) to scale at runtime.
            Form dialogForm = new Form
            {
                Text = "New Connection",
                Size = BeepLayoutMetrics.DialogMedium,
        ...
        private static Form CreateDialogForm(uc_DataConnectionBase connectionControl, string title)
        {
            ...
            {
                Text = title,
                // Skill § 1: dialog sizing flows from BeepLayoutMetrics; AutoScale handles DPI.
                Size = BeepLayoutMetrics.DialogMedium,
                StartPosition = FormStartPosition.CenterParent,
                FormBorderStyle = FormBorderStyle.FixedDialog,
                MaximizeBox = false,
